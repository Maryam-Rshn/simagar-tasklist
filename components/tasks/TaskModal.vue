<template>
  <Teleport to="body">
    <div v-if="open" class="fixed inset-0 z-50 flex items-end justify-end p-4">
      <button
        type="button"
        ...
        <form class="space-y-4" @submit.prevent="onSubmit">
          ...
          <div class="grid gap-3 sm:grid-cols-3">
            ...
            <div>
              ...
              <select v-model.number="form.assigneeId" class="field-input">
...
function resetForm() {
  ...
  form.assigneeId = props.task?.members?.[0] ?? null;
...
async function onSubmit() {
  ...
  if (!form.priority) {
    submitError.value = "Priority is required.";
    return;
  }

  if (!form.endDate) {
    submitError.value = "Due date is required.";
    return;
  }

  if (form.assigneeId === null) {
    submitError.value = "Assignee is required.";
    return;
  }
